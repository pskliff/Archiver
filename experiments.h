...
//                throw std::runtime_error("File does't exist (get_frequency())");
    ...
    void write_huff_to_csv(std::string compress_path, std::string coef_path)
    {
        ...
        for (int i = 1; i <= 36; ++i)
        {
            ...
            std::cout << "************ Entered  &&&&&&&&& FILE CYCLE &&&&&&&&&&   ****************" << file_ind << std::endl;
            ...
            for (int j = 0; j < 20; ++j)
            {
                ...
                std::cout << "************ Entered  TIMES cycle   ****************  iter = " << j << std::endl;
                ...
                after_comp = EncDec::decode_file(source_path + HUFF_ENCODING_EXTENCION,
                                                 source_path + HUFF_DECODING_EXTENCION);
            ...
            alg_times.push_back(
                    Utility::alg_time_row(comp_time * 1.0 / 20, decomp_time * 1.0 / 20, HUFF_TYPE, file_ind));
            compress_coefs.push_back(Utility::comp_coef_row(after_comp / before_comp, HUFF_TYPE, file_ind));
        }
    ...
    /**
    * Counts how much time in ns ShennonFano needs to compress and decompress
    * and also how much are the files compressed
    * and writes it in 2 csv files
    */
    void write_fano_to_csv(std::string compress_path, std::string coef_path)
    {
        std::cout << "\n******************** Entered  write_FANO_csv   ************************\n";
        std::string source_path;
        std::vector<Utility::comp_coef_row> compress_coefs;
        std::vector<Utility::alg_time_row> alg_times;
        double before_comp, after_comp;
        ...
        for (int i = 1; i <= 36; ++i)
        {
            ...
            std::cout << "************ ENTER  &&&&&&&&& FILE CYCLE &&&&&&&&&&   ****************"<< file_ind << std::endl;
            ...
            for (int j = 0; j < 20; ++j)
            {
                ...
                before_comp = EncDec::encode_file(source_path, source_path + FANO_ENCODING_EXTENCION, fano_codes);
                auto end = chrono::high_resolution_clock::now();

                comp_time += chrono::duration_cast<chrono::nanoseconds>(end - start).count();

                start = chrono::high_resolution_clock::now();
                after_comp = EncDec::decode_file(source_path + FANO_ENCODING_EXTENCION,
                                                 source_path + FANO_DECODING_EXTENCION);
                end = chrono::high_resolution_clock::now();
            ...
            alg_times.push_back(
                    Utility::alg_time_row(comp_time * 1.0 / 20, decomp_time * 1.0 / 20, FANO_TYPE, file_ind));
            compress_coefs.push_back(Utility::comp_coef_row(after_comp / before_comp, FANO_TYPE, file_ind));
        }
        ...
        std::cout << "******************** EXIT  write_FANO_csv   ************************\n";
    }


    /**
    * Counts how much time in ns LZ77 needs to compress and decompress
    * and also how much are the files compressed
    * and writes it in 2 csv files
    */
    void write_lz77_to_csv(std::string compress_path, std::string coef_path, int hist_buf_max, int prev_buf_max,
                           std::string extencion, std::string dec_extencion, std::string lz_type)
    {
        std::cout << "\n******************** Entered  write_FANO_csv   ************************\n";
        std::string source_path;
        std::vector<Utility::comp_coef_row> compress_coefs;
        ...
        for (int i = 1; i <= 36; ++i)
        {
            ...
            for (int j = 0; j < 20; ++j)
            {
                ...
                auto start = chrono::high_resolution_clock::now();
                std::vector<Node> lz77_codes = ut.create_lz77_codes(source_path, hist_buf_max, prev_buf_max, before_comp);
                EncDec::write_lz77(lz77_codes, source_path + extencion);
                auto end = chrono::high_resolution_clock::now();

                comp_time += chrono::duration_cast<chrono::nanoseconds>(end - start).count();

                start = chrono::high_resolution_clock::now();
                after_comp = EncDec::decode_lz77(source_path + extencion, source_path + extencion);
                end = chrono::high_resolution_clock::now();
            ...
            alg_times.push_back(
                    Utility::alg_time_row(comp_time * 1.0 / 20, decomp_time * 1.0 / 20, lz_type, file_ind));
            compress_coefs.push_back(Utility::comp_coef_row(after_comp / before_comp, lz_type, file_ind));
        }
